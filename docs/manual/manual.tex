...
\begin{enumerate}
  ...
  \item Unzip it into the program folder (next to \viewer{}), or just drop the zip on the
    program's window.
  \item Pick it in the \ui{Model} list, under the spectrogram (the \ui{Batch} view has the
    same list).
\end{enumerate}
...
\begin{figure}[H]
  \centering
  \begin{tikzpicture}
    \node[anchor=south west,inner sep=0] (img) {\includegraphics[width=0.9\textwidth]{spectrogram}};
    \begin{scope}[x={(img.south east)},y={(img.north west)}]
      \node[callout] at (-0.025,0.985) {1};
      \node[callout] at (-0.025,0.55)  {2};
      \node[callout] at (-0.025,0.117) {3};
      \node[callout] at (-0.025,0.071) {4};
      \node[callout] at (-0.025,0.02)  {5};
    \end{scope}
  \end{tikzpicture}
...
\begin{enumerate}[label=\protect\callout{\arabic*}, leftmargin=*]
  \item \textbf{Menus.} Everything the program does, with its keyboard shortcut: open,
    save, the two views (\ui{Spectrogram}, \ui{Batch}), zoom, \ui{Boxes}, \ui{Settings}.
  ...
  \item \textbf{Detection.} The \ui{Model}, \ui{Detect}, the \ui{Score} filter, the two
    layers and \ui{Review}.
...
\begin{enumerate}
  \item Press \ui{Detect}, under the spectrogram (\keys{\ctrl+R}).
...
\begin{figure}[H]
  \centering
  \shot{context}
  \caption{Below the spectrogram: \ui{Model} and \ui{Detect}, the \ui{Score} filter, the
    layers and \ui{Review}.}
\end{figure}
...
\menu{View > Boxes} (\keys{\ctrl+E}) opens the list of boxes. Click a row to go to that
box; \keys{N} and \keys{P} go to the next and previous one. Press \keys{Del} to remove it.
...
\begin{enumerate}
  \item \menu{View > Batch}.
  \item Drag a folder onto the window, or \ui{Choose a folder\ldots}
    (\menu{File > Open folder\ldots}, \keys{\ctrl+\shift+O}, does both).
  \item Click \ui{Run}.
\end{enumerate}
...
\begin{tabular}{@{}ll@{}}
  ...
  \keys{N} \keys{P} & next, previous box (its row is selected in \ui{Boxes}) \\
...
\begin{figure}[H]
  ...
  \caption{\menu{View > Settings\ldots}}
\end{figure}
...
\begin{itemize}
  \item \textbf{No sound}: \menu{View > Settings\ldots} $\to$ \ui{Audio output}, pick your
    device.
