...
del Capítulo~\ref{cap:curacion} deja 19\,029 anotaciones utilizables: es la
...
\begin{quote}
    ...
    proponga cajas tiempo--frecuencia con especie y tipo de llamada sobre grabaciones
    de PAM de primates amazónicos, con el fin de reducir el tiempo de revisión
    experta.
\end{quote}
...
El mismo patrón se repite fuera de primates: HOWLish recupera el 81,3\,\% de los
eventos de aullido observados y ofrece una reducción de 15 veces en el tiempo del
operador frente a la detección totalmente manual, y su tubería redujo 22 veces el
volumen de datos que un operador debía procesar manualmente
\citep{Campos_Krofel_Rio-Maior_Renna_2025}.
...
\begin{table}[htbp]
    ...
    \begin{tabular}{@{}L{1.0cm}L{3.7cm}L{3.2cm}L{6.1cm}@{}}
        ...
        \textbf{RE} & \textbf{Resultado} & \textbf{Medio de verificación} & \textbf{Indicador objetivamente verificable (IOV)} \\
        \midrule
        RE2.1 & Diseño experimental, arquitectura propuesta y línea base. & Documento de diseño y módulo de arquitecturas. & Especifica la arquitectura AST-Deformable-DETR implementada, con dimensión 128, 64 consultas y 3 niveles, sus hiperparámetros, y define la comparación con la línea base declarada. \\
        \addlinespace
...
\begin{table}[htbp]
    ...
    \begin{tabular}{@{}L{1.0cm}L{3.7cm}L{3.2cm}L{6.1cm}@{}}
        ...
        \textbf{RE} & \textbf{Resultado} & \textbf{Medio de verificación} & \textbf{Indicador objetivamente verificable (IOV)} \\
        \midrule
        RE3.1 & Metodología de validación comparativa. & Documento de metodología y protocolo de revisión experta. & Especifica el subconjunto de prueba, el punto de operación (confianza 0,5 y NMS IoU 0,3), el umbral IoU 0,5 para verdadero positivo, las métricas, el emparejamiento y el procedimiento para revisar discrepancias y eventos potencialmente omitidos. \\
        \addlinespace
...
\begin{description}
    \item[RE3.1.] Protocolo de la prueba: material, punto de operación,
          procedimiento y métricas, siguiendo el marco de tubería para identificación
          automatizada de sonidos \citep{gibb_2018}.

          % TODO METODOLÓGICO
          % Al diseñar RE3.2 hay que controlar el EFECTO DE APRENDIZAJE: un
          % revisor va más rápido la segunda vez que revisa un material aunque
          % no reciba ayuda. Usa material distinto pero comparable en cada
          % condición, o dos revisores con orden contrabalanceado. Declararlo
          % en RE3.1 evita que el jurado lo señale como falla de diseño.
          % =================================================================

    \item[RE3.2.] Medición del tiempo de revisión asistida frente a anotación
          manual sobre material comparable. \citet{heinicke_2015} reportan que
          en primates la remoción manual de falsos positivos tomó 3--5\,\% del tiempo
          frente a la recolección y el procesamiento humanos, y
          \citet{Campos_Krofel_Rio-Maior_Renna_2025} reportan una reducción de 15 veces
          en tiempo de operador y de 22 veces en volumen de audio a revisar. Son los dos
          precedentes metodológicos de esta medición.

          % TODO No comprometer un VALOR concreto de ahorro en el enunciado del
          % objetivo. "Reducir el tiempo de revisión" es un objetivo; "reducirlo
          % en 60 %" es una apuesta que puede hacer fracasar la tesis por una
          % cifra elegida antes de tener datos.

    \item[RE3.3.] Informe de revisión experta de falsos positivos de alta
          confianza, clasificados en evento no anotado, ruido o error de encuadre. Los
          métodos automáticos fallan con llamadas tenues o enmascaradas
          \citep{gibb_2018}, lo que implica la existencia de eventos válidos
          fuera del \textit{ground truth}; la variabilidad intra-analista
          \citep{leroy_reliability_2018} apunta en la misma dirección desde el lado
          humano.
\end{description}
...
\begin{table}[htbp]
    ...
    \begin{tabular}{@{}L{3.6cm}L{2.0cm}L{2.0cm}L{6.0cm}@{}}
        ...
        \midrule
        C1: Velocidad          & E1              & OE3               & RE3.1, RE3.2, RE3.3                         \\
        \addlinespace
...
\begin{table}[htbp]
    ...
    \begin{tabular}{@{}L{4.6cm}L{5.9cm}L{1.8cm}L{1.9cm}@{}}
        ...
        \addlinespace
        5. Evaluación               & Métricas por eje, análisis de errores y tiempo de revisión                   & OE2, OE3          & 5                 \\
        \addlinespace
...
con la reducción efectiva del tiempo de revisión experta, como recomienda el marco para
asegurar que el resultado sea útil y no solo correcto.
...
Lo que se compara, por tanto, no son familias de arquitectura sino dos
configuraciones que sí satisfacen la exigencia: la arquitectura propuesta
sobre un extractor de audio con cabeza de detección y una línea base de detector
sobre espectrogramas \citep{gonzalez_yolo-based_2025}.
...
\textbf{19\,029 anotaciones sobre ocho especies, repartidas en 65 pares
    distintos de especie y tipo de llamada}. De esos 65 pares, 41 pertenecen al
vocabulario declarado para el dominio y concentran 18\,693 anotaciones; los 24
restantes, que suman 336 registros (el 1,8\,\%), quedan fuera del vocabulario y se
...
\begin{table}[htbp]
    ...
    \begin{threeparttable}
        ...
        \begin{tabular}{@{}L{4.9cm}crrcr@{}}
            ...
            \midrule
            \textit{Leontocebus weddelli}  & LW            & 11                                          & 4\,103                                   & 1              & 2\,282              \\
            \textit{Sapajus macrocephalus} & SM            & 8                                           & 3\,605                                   & 2              & 2\,731              \\
            \textit{Alouatta sara}         & AS            & 2                                           & 3\,309                                   & 2              & 3\,309              \\
            \textit{Ateles chamek}         & AC            & 6                                           & 3\,081                                   & 1              & 2\,487              \\
            \textit{Saimiri boliviensis}   & SB            & 5                                           & 2\,324                                   & 2              & 1\,880              \\
            \textit{Plecturocebus toppini} & PT            & 5                                           & 1\,155                                   & 1              & 637                 \\
            \textit{Aotus azarae}          & AA            & 3                                           & 1\,087                                   & 1              & 799                 \\
            \textit{Cebus cuscinus}        & CC            & 1                                           & 29                                       & 0              & N/D                 \\
            \midrule
            \textbf{Total}                 &               & \textbf{41}                                 & \textbf{18\,693}                         & \textbf{10}    & \textbf{14\,125}    \\
            \bottomrule
        \end{tabular}
        \begin{tablenotes}[flushleft]
            \footnotesize
            \item Las 336 anotaciones restantes hasta 19\,029 corresponden a los
            24 pares fuera de vocabulario, marcados para revisión.
        \end{tablenotes}
    \end{threeparttable}
\end{table}
...
larga: diez pares concentran el 74\,\% de las 19\,029 anotaciones y el resto se
reparte hasta pares con una sola.
...
Tabla~\ref{tab:geometria} lo mide sobre las 19\,029 anotaciones.
...
\begin{figure}[htbp]
    ...
    \caption[Geometría de las clases]{Dónde vive cada clase en tiempo y en frecuencia. Las especies
        ...
        curado con suficientes anotaciones para caracterizarlos, un conjunto más
        amplio que las diez clases del experimento
        (Tabla~\ref{tab:clases-experimento}) y que corresponde al vocabulario
        ampliado de la sección~\ref{sec:revision}.}
...
Los pares que caen fuera del vocabulario, 24 pares que suman 336 registros, no se
...
Sobre el vocabulario se recorta el subconjunto de experimento en dos pasos:

\begin{enumerate}[nosep]
    \item Se excluyen las dos clases de frase, \texttt{lw/cc} y \texttt{sm/fc},
          por la razón de anidamiento de la sección~\ref{sec:silabas}.
    \item Se conservan los pares con al menos 500 anotaciones, umbral por debajo
          del cual una clase no sostiene ni entrenamiento ni evaluación por clase.
\end{enumerate}

El resultado son las \textbf{10 clases y 14\,125 anotaciones} de la
Tabla~\ref{tab:clases-experimento}, distribuidas sobre siete de las ocho especies:
CC no alcanza el umbral con su única llamada de 29 anotaciones y queda fuera del
experimento, aunque permanece en el conjunto curado.
...
\begin{table}[htbp]
    ...
    \begin{threeparttable}
        ...
        \begin{tabular}{@{}L{2.2cm}L{5.6cm}rL{4.0cm}@{}}
            \toprule
            \textbf{Clase}             & \textbf{Tipo de llamada} & \thead{Anot.}    & \textbf{Rol}               \\
            \midrule
            \texttt{ac/bc}             & Ladrido                  & 2\,487           & Experimento                \\
            \texttt{lw/cs}             & Sílaba de contacto       & 2\,282           & Experimento                \\
            \texttt{sm/cc}             & Llamada de contacto      & 2\,040           & Experimento                \\
            \texttt{as/bc}             & Ladrido                  & 1\,909           & Experimento                \\
            \texttt{as/hc}             & Aullido                  & 1\,400           & Experimento                \\
            \texttt{sb/ppc}            & Llamada de juego         & 1\,151           & Experimento                \\
            \texttt{aa/gc}             & Llamada de trago         & 799              & Experimento                \\
            \texttt{sb/spc}            & Llamada de escupido      & 729              & Experimento                \\
            \texttt{sm/fs}             & Sílaba de comida         & 691              & Experimento                \\
            \texttt{pt/dc}             & Dueto                    & 637              & Experimento                \\
            \midrule
            \texttt{lw/cc}             & Llamada de contacto      & 536              & Excluida: contiene sílabas \\
            \texttt{sm/fc}             & Llamada de comida        & 126              & Excluida: contiene sílabas \\
            \midrule
            \textbf{Total experimento} &                          & \textbf{14\,125} &                            \\
            \bottomrule
        \end{tabular}
        \begin{tablenotes}[flushleft]
            \footnotesize
            \item \texttt{lw/cc} supera el umbral de 500 y habría entrado en el
            experimento; se retira por anidamiento
            (sección~\ref{sec:silabas}), no por escasez. \texttt{sm/fc} no
            alcanza el umbral, pero se consigna aquí porque su exclusión
            responde al mismo criterio y porque \texttt{sm/fs}, que sí está en
            el experimento, vive dentro de ella.
        \end{tablenotes}
    \end{threeparttable}
\end{table}
...
70/10/20, con semilla fijada para que la partición sea reproducible. La división
...
\subsection{El ventaneo y su defecto}
...
1,5\,s. Una anotación se asigna a una ventana solo si al menos la mitad de su
duración cae dentro de ella.

Esa regla tiene una consecuencia que conviene enunciar sin rodeos: \textbf{ningún
    evento de más de 6\,s puede asignarse a ninguna ventana}, porque no existe
posición de la ventana en la que la mitad de su duración quepa dentro. El efecto
se concentra en las dos clases de eventos largos y elimina \textbf{1\,514
    anotaciones, el 10,7\,\% del subconjunto de experimento}, antes de que el
entrenamiento comience (Tabla~\ref{tab:inalcanzables}).

\begin{table}[htbp]
    \centering
    \small
    \begin{threeparttable}
        \caption{Anotaciones que el ventaneo de 3\,s vuelve inalcanzables}
        \label{tab:inalcanzables}
        \begin{tabular}{@{}L{2.4cm}rrr@{}}
            \toprule
            \textbf{Clase} & \thead{Anotaciones} & \thead{Inalcanzables                   \\($>6$\,s)} & \thead{Fracción\\perdida} \\
            \midrule
            \texttt{as/hc} & 1\,400              & 1\,147               & 82\,\%          \\
            \texttt{pt/dc} & 637                 & 367                  & 58\,\%          \\
            \midrule
            \textbf{Total} & \textbf{2\,037}     & \textbf{1\,514}      & \textbf{74\,\%} \\
            \bottomrule
        \end{tabular}
        \begin{tablenotes}[flushleft]
            \footnotesize
            \item Ninguna otra clase del experimento pierde una fracción
            apreciable. Un 13,5\,\% adicional del subconjunto excede los 3\,s de
            la ventana y sobrevive únicamente como caja recortada al borde.
        \end{tablenotes}
    \end{threeparttable}
\end{table}


Este defecto se documenta aquí, y no en el apartado de limitaciones, porque
determina la lectura de los resultados por clase del
Capítulo~\ref{cap:deteccion}: el \textit{recall} de \texttt{as/hc} y
\texttt{pt/dc} está acotado por el preprocesamiento antes de que intervenga
ninguna decisión de arquitectura. Es la manifestación concreta del rango de
2\,134$\times$ en duración de la Tabla~\ref{tab:geometria}: la ventana de 3\,s
acomoda ocho de las diez clases y sacrifica las dos restantes. Su corrección
Una segunda longitud de ventana para las clases largas, o un criterio de
solapamiento relajado con cajas recortadas al borde, es el principal pendiente
de esta etapa.
...
mediana es 2 y el máximo observado, 11. Es una propiedad del conjunto derivado,
pero se consigna aquí porque el Capítulo~\ref{cap:deteccion} la usa para
dimensionar el número de predicciones simultáneas que el decodificador debe
sostener.
...
Tabla~\ref{tab:preproc}. Log-mel es la representación con la que se construye el
conjunto evaluado; la comparación contra la normalización de energía por canal
anunciada en la sección~\ref{sec:representacion} está pendiente, y su
implementación ya existe en el repositorio pero no se ha medido bajo el criterio
declarado allí. La columna de justificación registra qué falla con el
...
\begin{table}[htbp]
    ...
    \begin{tabular}{@{}L{2.6cm}L{3.0cm}L{8.3cm}@{}}
        ...
        Acomoda 8 de las 10 clases; el costo son las dos clases largas de la
        Tabla~\ref{tab:inalcanzables}                                             \\
        \addlinespace
        Solapamiento mínimo      & 0,5                   &
        Un evento entra en una ventana solo si la mitad de su duración cae dentro \\
        \addlinespace
...
\section{Revisión posterior del vocabulario}
\label{sec:revision}

La configuración descrita hasta aquí, con diez clases y un umbral de 500
anotaciones, es la que se evalúa en el Capítulo~\ref{cap:deteccion}, y todas
las cifras de desempeño de esta tesis corresponden a ella. Esta sección documenta
una revisión posterior del vocabulario, motivada por dos limitaciones que la
propia ficha del conjunto derivado había registrado.

\rotulo{Qué motivó la revisión.} La primera limitación es de cobertura: con el
umbral en 500, veintinueve tipos de llamada que sí pertenecen al vocabulario del
dominio quedaban sin usar, y una especie completa, CC, fuera del experimento.
La segunda es de granularidad, y proviene del análisis geométrico de la
sección~\ref{sec:geometria}: la familia de trinos de LW (\texttt{tr},
\texttt{tj}, \texttt{tt} y \texttt{tf}) comparte banda y duración casi por
completo, de modo que mantenerlos como cuatro clases distintas obliga al detector
a separar lo que la geometría no separa, y reparte entre cuatro etiquetas escasas
un material que como clase única sí alcanza masa suficiente.

\rotulo{Qué cambió.} Tres modificaciones sobre la selección, ninguna sobre el
protocolo de curación de la sección~\ref{sec:protocolo}:

\begin{enumerate}[nosep]
    \item El umbral de selección baja de 500 a 100 anotaciones por par.
    \item Los cuatro tipos de trino de LW se fusionan en una clase única,
          \texttt{lw/trino}.
    \item Se añade \texttt{sb/pcs} a los pares excluidos.
\end{enumerate}

El resultado son \textbf{19\,042 anotaciones sobre 54 pares distintos, de los
    cuales 25 superan el umbral y concentran el 92\,\% del material}
(Figura~\ref{fig:annotations-per-pair}), frente al 74\,\% que cubrían las diez
clases anteriores. La cobertura del experimento pasa de siete especies a las
ocho.

% TODO CHANGELOG DEL CONJUNTO
% El total de anotaciones curadas pasa de 19 029 a 19 042 (+13) y el
% número de pares de 65 a 54. La fusión de los cuatro trinos y la
% exclusión de sb/pcs explican parte de la caída de pares, pero no
% toda, y el +13 en anotaciones no está documentado en ningún sitio:
% procede de cambios en el mapa de sinónimos posteriores al reporte.
% Registra en la ficha del conjunto qué reglas se añadieron entre una
% versión y otra; sin ese registro las dos cifras no se reconcilian.
% ===================================================================

\begin{figure}[htbp]
    \centering
    \includegraphics[height=0.80\textheight]{figures/annotations_per_pair.png}
    \caption[Anotaciones por par tras la revisión]{Anotaciones por par de especie y tipo de llamada tras la revisión
        del vocabulario. Las 25 clases por encima del umbral de 100 concentran el
        92\,\% de las 19\,042 anotaciones; la cola larga llega hasta pares con una
        sola.}
    \label{fig:annotations-per-pair}
\end{figure}

\rotulo{Qué mejora y qué no.} La ampliación resuelve la cobertura pero no toca
las dos causas estructurales que este capítulo documentó. El desbalance no
desaparece: se traslada al nuevo umbral, y entre la clase mayoritaria y las que
apenas lo superan sigue habiendo más de un orden de magnitud
(Figura~\ref{fig:boxes-split}). Y el defecto
de ventaneo permanece intacto, porque no depende del vocabulario sino de la
longitud de la ventana: \texttt{as/hc} sigue perdiendo el 82\,\% de sus
anotaciones y \texttt{pt/dc} el 58\,\%, con tres clases más aportando pérdidas
menores (Figura~\ref{fig:unreachable}). La corrección de ese defecto sigue siendo
el principal pendiente de la etapa.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{figures/boxes_per_split.png}
    \caption[Cajas por clase y partición]{Cajas por clase y partición bajo el vocabulario ampliado. El balance
        se preserva entre particiones; el desbalance entre clases se mantiene, ahora
        sobre veinticinco.}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{figures/unreachable_events.png}
    \caption[Anotaciones alcanzables por el ventaneo]{Fracción de anotaciones que cada clase puede aportar bajo el
        ventaneo de 3\,s, con el vocabulario ampliado. El defecto no depende del
        vocabulario: \texttt{as/hc} y \texttt{pt/dc} siguen perdiendo la mayor parte
        de su material.}
    \label{fig:unreachable}
\end{figure}

Una consecuencia menos evidente afecta al dimensionado del decodificador. Al
incorporar clases de eventos más aislados, la densidad por ventana baja: la
mediana de cajas por ventana de 3\,s pasa de 2 a 1, aunque el máximo se mantiene
en 11 (Figura~\ref{fig:boxes-window}). El presupuesto de predicciones simultáneas
que fija el Capítulo~\ref{cap:deteccion} sigue siendo válido, porque se dimensiona
sobre el máximo y no sobre la mediana.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.92\textwidth]{figures/boxes_per_window.png}
    \caption[Cajas anotadas por ventana]{Cajas anotadas por ventana de 3\,s bajo el vocabulario ampliado:
        mediana 1, máximo 11.}
    \label{fig:boxes-window}
\end{figure}

El conjunto derivado bajo esta configuración está construido,
pero el detector no se ha entrenado ni evaluado sobre él. Los resultados del
Capítulo~\ref{cap:deteccion} corresponden a las diez clases, y extenderlos a las
veinticinco es trabajo pendiente: no basta con reentrenar, porque buena parte de
las clases incorporadas tiene menos cajas de entrenamiento que la más escasa de
las diez actuales, y el análisis por clase deberá distinguir entre una caída
atribuible al modelo y una atribuible a la escasez de la clase.
...
\begin{itemize}[nosep]
    \item Las 1\,514 anotaciones inalcanzables bajo el ventaneo de 3\,s
          (Tabla~\ref{tab:inalcanzables}), y el 13,5\,\% adicional que sobrevive
          recortado al borde de la ventana.
    \item Las dos clases de frase excluidas por anidamiento
          (sección~\ref{sec:silabas}), que dejan sin cubrir el nivel superior de la
          jerarquía vocal.
    \item Los 29 tipos de llamada en vocabulario que no alcanzan el umbral de
          500 anotaciones y quedan fuera del experimento, entre ellos la única clase de
          CC, que hace que el conjunto de experimento cubra siete especies y no ocho;
          la sección~\ref{sec:revision} documenta la revisión que responde a esta
          limitación.
    \item Los 24 pares fuera de vocabulario (336 registros) pendientes de
...
% TODO Redactar.
% TODO TABLA 5.4: resultados por eje, propuesta vs. línea base.
% TODO TABLA 5.5: estudio de ablación acumulativo.
% TODO FIGURA 5.2: recall por clase con la causa atribuida a cada caída.
% TODO FIGURA 5.3: ejemplos cualitativos de detección, incluyendo AL
%                  MENOS UN CASO DIFÍCIL (escena densa con especies
%                  solapadas). Mostrar solo los aciertos debilita el
%                  capítulo ante el jurado.
...
\begin{itemize}[nosep]
    \item Reducir el tiempo de revisión experta por hora de audio analizada.
...
la comparación cuantitativa del tiempo de revisión asistida frente al análisis
manual. El proyecto recorre el ciclo completo de CRISP-DM, desde la preparación
de los datos hasta el procesamiento de grabaciones nuevas.
...
\begin{table}[htbp]
    ...
    \begin{tabular}{@{}L{1.5cm}L{12.9cm}@{}}
        ...
        \addlinespace
        RE3.1            & El protocolo especifica el material de prueba, el punto de operación, el umbral de IoU que define un verdadero positivo y el procedimiento de revisión experta, incluyendo el control del efecto de aprendizaje. \\
        \addlinespace
        RE3.2            & Se reporta el tiempo de revisión en ambas condiciones sobre material comparable, junto con el tiempo de procesamiento del modelo por hora de audio.                                                              \\
        \addlinespace
        RE3.3            & El informe clasifica la muestra de falsos positivos de alta confianza en evento no anotado, ruido y error de encuadre, con ejemplos visualizados sobre el espectrograma.                                         \\
        \bottomrule
    \end{tabular}
\end{table}
...
sensores de bajo costo añaden variabilidad entre dispositivos.
...
\begin{table}[htbp]
    ...
    \begin{tabular}{@{}L{0.8cm}L{4.4cm}L{6.6cm}L{2.2cm}@{}}
        ...
        6.3         & Protocolo de la prueba de tiempo & Material, punto de operación, procedimiento y métricas                                             & RE3.1               \\
        6.4         & Medición del tiempo de revisión  & Revisión asistida frente a anotación manual                                                        & RE3.2               \\
        6.5         & Revisión experta de errores      & Clasificación de una muestra de falsos positivos de alta confianza                                 & RE3.3               \\
        \addlinespace
...
modelado, el modelado precede a la evaluación, y la medición de tiempo de
revisión precede al análisis de errores, porque se realiza sobre las
predicciones del modelo ya seleccionado.
...
\begin{table}[htbp]
    ...
    \begin{tabular}{@{}L{2.8cm}ccL{4.4cm}L{4.2cm}@{}}
        ...
        Media                                                                 & Media         &
        Contactar a los revisores con antelación; protocolo de revisión breve y
        de carga acotada                                                      &
        Validación interna con el asesor, declarada de forma transparente como
