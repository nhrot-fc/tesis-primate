...
LW & 11 & \texttt{cc} (contact call), \texttt{cs} (contact syllable), \texttt{aa} (aerial alarm call), \texttt{ta} (terrestrial alarm call), \texttt{sqc} (squeal call), \texttt{phc} (phee call), \texttt{tr} (trino call), \texttt{tf} (trino fast call), \texttt{tj} (unofficial tj call), \texttt{tt} (trino transition call), \texttt{vc} (visual contact call) \\
PT & 5 & \texttt{dc} (duet call), \texttt{ac} (alarm call), \texttt{bp} (unofficial bellow phrase), \texttt{pp} (unofficial pant phrase), \texttt{sqc} (unofficial squeal call) \\
